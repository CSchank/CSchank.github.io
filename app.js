...
var $author$project$AudioApp$UserMsg = function (a) {
	return {$: 'UserMsg', a: a};
};
...
var $author$project$AudioApp$play = _Platform_outgoingPort('play', $elm$core$Basics$identity);
var $elm$json$Json$Encode$string = _Json_wrap;
var $author$project$AudioApp$cmdifySound = function (sCmd) {
	if (sCmd.$ === 'PlaySound') {
		var url = sCmd.a;
		return $author$project$AudioApp$play(
			$elm$json$Json$Encode$string(url));
	} else {
		return $elm$core$Platform$Cmd$none;
	}
};
var $author$project$AudioApp$hiddenAudioUpdate = F3(
	function (userUpdate, msg, model) {
		var userMsg = msg.a;
		var _v1 = A2(userUpdate, userMsg, model);
		var newModel = _v1.a;
		var soundCmd = _v1.b;
		return _Utils_Tuple2(
			newModel,
			$author$project$AudioApp$cmdifySound(soundCmd));
	});
var $elm$core$Platform$Sub$none = $elm$core$Platform$Sub$batch(_List_Nil);
var $author$project$AudioApp$audioApp = F2(
	function (tickMsg, userApp) {
		var userView = userApp.view;
		var userUpdate = userApp.update;
		...
		return A2(
			...
			F2(
				function (time, keystate) {
					return $author$project$AudioApp$UserMsg(
						A2(tickMsg, time, keystate));
				}),
			{
				init: function (_v0) {
					return _Utils_Tuple2(userInit, $elm$core$Platform$Cmd$none);
				},
				subscriptions: function (_v1) {
					return $elm$core$Platform$Sub$none;
				},
				update: $author$project$AudioApp$hiddenAudioUpdate(userUpdate),
				view: function (userModel) {
					return {
						body: A2(
							$MacCASOutreach$graphicsvg$GraphicSVG$mapCollage,
							$author$project$AudioApp$UserMsg,
							userView(userModel)),
						title: userApp.title
					};
				}
...
var $author$project$AudioApp$NoSound = {$: 'NoSound'};
var $author$project$AudioApp$PlaySound = function (a) {
			...
			case 'Tick':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{time: t}),
					$author$project$AudioApp$NoSound);
			case 'ClickedLeftButton':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{counter: model.counter - 1}),
					$author$project$AudioApp$PlaySound('Sounds/failure.wav'));
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{counter: model.counter + 1}),
					$author$project$AudioApp$PlaySound('Sounds/success.wav'));
		}
	});
var $author$project$Main$ClickedLeftButton = {$: 'ClickedLeftButton'};
var $author$project$Main$ClickedRightButton = {$: 'ClickedRightButton'};
		...
		_List_fromArray(
			[
				...
				A2(
					...
					$author$project$Main$ClickedRightButton,
					A2(
						$MacCASOutreach$graphicsvg$GraphicSVG$filled,
				...
				A2(
					$MacCASOutreach$graphicsvg$GraphicSVG$notifyTap,
					$author$project$Main$ClickedLeftButton,
					A2(
						$MacCASOutreach$graphicsvg$GraphicSVG$filled,
...
var $author$project$Main$main = A2(
	$author$project$AudioApp$audioApp,
	$author$project$Main$Tick,
	{
		model: {counter: 0, time: 0},
